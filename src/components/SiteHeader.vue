      ...
      <div class="head-search head-search-bijia">
        <div class="panel">
          <div class="sInput" style="position: relative; z-index: 0;">
            <input type="text" class="skey" style="width:575px;" placeholder="请输入想要优惠比较的商品名称" v-model="$store.state.search.searchValue"
              @keyup.enter="search" />
          </div>
          <div class="sBtn">
            <router-link to="/search">
              <input type="button" value="搜索" name="btnSearch" class="btnSearch" @click="search"/>
            </router-link>
          </div>
        </div>
      </div>
...
<style scoped>
  ...
  .new-users ul li i {
    display: block;
    background: url(../assets/images/header-total.gif) no-repeat;
  }
  ...
  .header .mainNav .nav .navigation li a .cur {
    background: url(../assets/images/cur.gif) no-repeat;
    width: 7px;
    height: 4px;
